...
function ParentList() {
  ...
  const fetchAvailableStudents = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/parent/students/available`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      let studentList = data.students || data || [];

      // ✅ FIX: include already linked student in edit mode
      if (editMode && selectedParent?.studentId) {
        const exists = studentList.find(
          (s) => s._id === selectedParent.studentId._id
        );

        if (!exists) {
          studentList.unshift(selectedParent.studentId);
        }
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const url = editMode
        ? `${API_URL}/api/parent/${selectedParent._id}`
        : `${API_URL}/api/parent`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/parent/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
