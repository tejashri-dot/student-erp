...
//   const token = localStorage.getItem("token");
//   const API_URL = "https://school-backend-6udp.onrender.com/api/attendance";
...
//       setError(null);
//       try {
//         const res = await api.get(`/students/by-class?className=${selectedClass}`);
...
//       setError(null);
//       try {
//         // Fetch all attendance records for this class and date
//         const res = await api.get("/students", {
//           params: { className: selectedClass, date: selectedDate },
//         });
//         const records = res.data; // array of attendance records with userId populated
//         const statusMap = {};
//         records.forEach((record) => {
//           const studentId = record.userId?._id;
//           if (studentId) {
//             statusMap[studentId] = record.status; // "Present" or "Absent" (or "Not Marked" if stored)
//           }
//         });
...
//         userType: "Student",
//         date: selectedDate,
//         status: status, // "Present", "Absent", or "Not Marked"
//       };
//       console.log("Sending attendance payload:", payload);
//       const response = await api.post("/mark", payload);
...
//     await markAttendance(selectedStudent._id, editStatus);
//     setEditDialogOpen(false);
//   };

//   const handleClearConfirm = async () => {
//     if (!selectedStudent) return;
//     await markAttendance(selectedStudent._id, "Not Marked");
...
//   const handleEditClick = (student) => {
//     setSelectedStudent(student);
//     setEditStatus(attendanceStatus[student._id] || "Not Marked");
//     setEditDialogOpen(true);
//   };

//   const handleClearClick = (student) => {
//     setSelectedStudent(student);
...
//                       <TableCell align="center">
//                         <Tooltip title="Edit">
//                           <IconButton size="small" onClick={() => handleEditClick(s)} sx={styles.editBtn}>
//                             <EditIcon fontSize="small" />
//                           </IconButton>
//                         </Tooltip>
//                         <Tooltip title="Clear">
//                           <IconButton size="small" onClick={() => handleClearClick(s)} sx={styles.deleteBtn}>
...
//           </Typography>
//           <Typography variant="body2" sx={{ mb: 2 }}>
//             Class: {selectedStudent?.className} | Seat No: {selectedStudent?.seatNumber}
//           </Typography>
//           <FormControl fullWidth sx={{ mt: 1 }}>
//             <InputLabel>Status</InputLabel>
...
//         <DialogTitle>Clear Attendance</DialogTitle>
//         <DialogContent>
//           <Typography variant="body1">
//             Are you sure you want to clear the attendance record for{" "}
//             <strong>{selectedStudent?.name}</strong>? This will set it to "Not Marked".
//           </Typography>
//         </DialogContent>
//         <DialogActions>
...
import {
  ...
  InputLabel, Select, MenuItem, Box, IconButton,
  Dialog, DialogTitle, DialogContent, DialogActions,
  Chip, Avatar, Tooltip, TablePagination,
...
/* ================= STYLES (UNCHANGED) ================= */
const styles = { /* KEEP YOUR SAME STYLES HERE (NO CHANGE) */ };
...
export default function StudentAttendance() {
    ...
    const fetchAttendance = async () => {
      ...
      try {
        const res = await api.get(
          `/students?className=${selectedClass}&date=${selectedDate}`
        );

        const map = {};
        res.data.forEach((item) => {
          if (item.userId?._id) {
            map[item.userId._id] = item.status;
          }
        });
  ...
  return (
    <Box sx={{ padding: "24px" }}>
      <Typography variant="h5" mb={2}>Student Attendance</Typography>
              ...
              <TableRow key={s._id}>
                ...
                <TableCell>
                  <IconButton onClick={() => {
                    ...
                    setEditStatus(attendanceStatus[s._id] || "Not Marked");
                    setEditDialogOpen(true);
                  }}>
                    <EditIcon />
                  </IconButton>
      ...
      <Dialog open={clearDialogOpen} onClose={() => setClearDialogOpen(false)}>
        <DialogTitle>Clear Attendance?</DialogTitle>
        <DialogActions>
          <Button onClick={handleClearConfirm}>Yes</Button>
        </DialogActions>
      </Dialog>
